...
from torch.utils.data.dataset import Dataset #Pytorch 里面定义的datasets 是instance of class
import numpy as np
...
def custom_collate_fn(data):
    ...
    :return:
    """
    # Extracts the tensors from each sample using a list comprehension
    # COllects the first three elements, assuming they are tensors
    tensors = [d[:3] for d in data]
    # Collects the fourth element of each sample, assuming it is a numpy array
    np_arrs = [d[3] for d in data]
    # stacking the tensors and creating the back, it handles the padding and tensor conversion
    coll_features, coll_feature_lens, coll_labels = default_collate(tensors)
    # Vertically stack numpy arrays
    np_arrs_coll = np.row_stack(np_arrs)
    

    return coll_features, coll_feature_lens, coll_labels, np_arrs_coll

    # By using this custom collate function, you can ensure that the meta data, 
    # represented as numpy arrays, are not processed with the standard collate function, 
    # but are instead kept separate and stacked into a single numpy array.
