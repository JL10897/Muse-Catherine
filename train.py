import time
import os 
from shutil import rmtree # recursively removing a dictionary
from typing import List, Dict # for type hintts
...
# DataLoader --> 在Dataset外边，是个iterator

from config import device, PERSONALISATION
from eval import evaluate


# performs the training loop for a single epoch and returns the training loss
def train(model, train_loader, optimizer, loss_fn, use_gpu=False):
    ...
    for batch, batch_data in enumerate(train_loader, 1):
        ...
        optimizer.zero_grad()
        # Just to clear out and initiate gradient

        preds,_ = model(features, feature_lens)
        # Input to the function "model": features & feature_lens
        # Output (that we care about): pred --> The prediction made by the model


        loss = loss_fn(preds.squeeze(-1), labels.squeeze(-1), feature_lens)
        # "pred.squeeze(-1)" is used to remove any singleton dimensions at the last axis, done to ensure that "preds" and "labels" have the same shape
        # and can be compared
        # calculate and returns the loss value

        loss.backward()
        # perform backpropagation
        optimizer.step()
        # updates the parameters using the chosen optimizer

        report_loss += loss.item() * batch_size
        # accumulates the loss values for current report
...
# saves the model to a file and returns the path of the saved model file
...
# perform the training and evaluation loop for the specified number of epochs, saves the best model and 
# returns the best validation loss, best validation score, and the path of the saved best model file
...
# performs personalized training of models for different subjects based on the given data loaders and other parameters, 
# it saves he personlized models and returns a dictionary mapping subject ID to their correspdongin best model file paths
